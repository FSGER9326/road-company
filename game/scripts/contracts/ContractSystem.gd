...
func can_accept(company: CompanyState, contract: Dictionary) -> bool:
	return not company.has_active_contract() and not contract.is_empty()

func accept(company: CompanyState, contract: Dictionary) -> bool:
	if not can_accept(company, contract):
		return false
	company.accept_contract(contract)
	return true

func active_contract_completed(company: CompanyState, route: Dictionary, destination: String) -> bool:
...
func complete(company: CompanyState) -> Dictionary:
	return company.apply_contract_success()

func fail(company: CompanyState) -> Dictionary:
